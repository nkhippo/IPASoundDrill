...
- PR を**マージしない**（マージ判断は Naoya のみ）。`main` へ直接 push しない。
...
- base ブランチを決定（Track A は `main`）。作業ブランチを作成。
...
2. push。
3. `gh pr create` で base（Track A は `main`）に PR を作成。`Closes #NN`（Track A のみ）・現行の PR description フォーマット・適切なラベルを付ける。
...
## やってはいけないこと（再掲）
PR のマージ / `main` への直接 push / 他リポ操作 / URL の推測生成 / 検証スキップ / スコープ拡大 / 憶測実装。
