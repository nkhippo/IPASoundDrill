...
`.github/PULL_REQUEST_TEMPLATE.md` を必ず使用する。PR description の Single source of truth は `.github/` 側とし、テンプレート更新時は AGENTS.md に本文を重複させない。

## Runtime UI 動作検証

Runtime UI やインタラクション (event handler, form 送信, DOM 変更, React state 等) を含む L2 以上の改修では、ブラウザでの実行確認を必須とする。

- 静的サイトはローカル HTTP server、React / Vite 等は `npm run dev` など、実際の runtime に近い方法で確認する
- 実装レポートに「ブラウザで確認した具体操作」を列挙する (例: どのボタン押下 → どの表示変化、console warning/error のゼロ確認)
- `npm run build` 成功のみを Runtime UI 動作確認と呼ばない
- docs 純粋変更、CI 設定、GitHub metadata など、Runtime UI に触れない改修は対象外

## md5 baseline 検証

堅固化パターン A / B / C の md5 baseline と検証手順は `docs/DEV-GUARDRAILS.md` § 4 に従う。

- パターン A (既存編集ゼロ) を宣言した改修では、Phase 0 で既存 tracked files の md5 baseline を取得し、Phase 完了時に既存ファイルが全一致することを確認する
- パターン B / C では、Issue のホワイトリスト外が不変であることを md5 diff で確認する
- 実装レポートに「Phase 0 md5 baseline 取得」「Phase 完了時 md5 検証結果」を記載する

## Scaffolding tool noise cleanup

`npm create`, `yarn create`, `create-*` 系のスキャフォールディングツールで生成された成果物のうち、Issue のスコープに関係ないものは commit 前に削除する。

- 例: `create-vite` の `public/icons.svg` (他社製品宣伝アイコン)、プロジェクト固有 favicon がある場合の template `public/favicon.svg`、README template placeholder
- 判断基準は「この Issue の本旨に必要か」。迷う場合は Issue Comment で確認し、不要と判断できる template noise は残さない

## Step 2 design concern comment format (agent-agnostic)

実装開始前に、Issue 本文に複数解釈があり得る判断ポイント、UX / 運用 / データ整合性の懸念、Runtime contract への疑義があれば、Issue Comment で設計懸念点検結果を投稿する。判断ポイントがない単純 Issue では省略可。
...
| # | 対象ファイル・箇所 | 解釈A | 解釈B | <agent> の採用案 |
|---|---|---|---|---|
| 1 | ... | ... | ... | ... |

### カテゴリ B: UX・運用・データ整合性の懸念
（該当があれば表を貼る。なければ「なし」）

| # | 懸念内容 | 現状の動作・運用 | 変更後の動作・運用 | 影響度 |
|---|---|---|---|---|
| 1 | ... | ... | ... | 大/中/小 |
...
`<agent>` は `Codex` / `Cursor` / `Claude Code` など、実行エージェント名を入れる。カテゴリ A が 1 件以上、またはカテゴリ B の影響度「大」が 1 件以上ある場合は、実装を進めず判断を待つ。
