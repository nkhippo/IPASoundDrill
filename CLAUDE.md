...
- 本ファイル(CLAUDE.md): 実装ルール・Issue 起票・品質基準(Claude / Cursor 共通)
- `docs/claude-collaboration.md`: Claude が相談相手として振る舞う際の、このプロジェクト固有のルール
- `docs/dev-common.md`: 開発運用型プロジェクト共通の横断ルール(Issue 起票詳細・PR Rv 12観点・判断相談フォーマット等)

**取得方法は環境依存**: Claude Chat では Vault MCP / github-mcp-cfw 経由、Claude Code ではローカル clone から参照する。どの環境でも同じファイルを読む。
...
- **Claude Code**: MCP は使わず、ローカル clone を直接参照・編集する(2026-07-25 時点)
...
2. `CLAUDE.md` を確認(Chat では MCP 経由、Claude Code ではローカル)
3. `docs/REPOSITORY-STRUCTURE.md` を確認
4. `docs/LAUNCH-CHECKLIST.md` を確認
5. `docs/DOCUMENT-MAP.md` を確認
6. Issue 起票・改修方針の議論では、加えて `docs/CHANGE-CLASSIFICATION.md` を確認する
7. 上記を整合させて現状把握
8. 憶測での回答を避け、不明な点は参照するか Naoya さんに確認
...
| 要件・仕様・Issue草稿 | Claude | アイデア整理・要件定義・Issue 本文草稿(.md)作成・Issue 起票 |
...
├── docs/claude-collaboration.md     ← Claude 相談ルール(プロジェクト固有)
├── docs/dev-common.md               ← 開発運用型プロジェクト横断ルール
...
│   └── archive/                     ← ローカル退避
│
├── docs/                            ← 正本ドキュメント + AI 履歴 + Vault 由来の相談履歴
│   ├── README.md                    ← docs/ 索引
...
│   ├── claude-design/               ← CD(Claude Design)出力の canonical source(HTML/CSS/JS モック)
│   ├── design/                      ← 設計思想・企画資料(Vault design/ 由来、personas/mood-board 等)
│   ├── handoff/                     ← Vault 由来の Chat 引き継ぎ書
│   ├── logs/                        ← Vault 由来の議論ログ
│   ├── vault-history/               ← Vault 由来の旧 project_instructions・backlog
│   ├── cursor/                      ← AI タスク履歴
│   │   ├── README.md
...
Step 1: 要件整理(Naoya × Claude)
  Naoya が Chat で相談 → Claude が要件を整理 → 5項目チェックを満たす Issue を起票
...
|---|---|---|---|
| Claude が起票 | Claude | Claude(起票時に同時付与) | Issue 起票時 |
...
Issue 本文には、Category C(Issue 起票時参照)および Category D(Cursor 実装時参照)から該当するドキュメントを列挙する。UI 改修 Issue では Category F(CD 修正判定)も「改修分類」ブロックに記載する。分類は `docs/DOCUMENT-MAP.md` § 2 を参照。
...
| Chat 相談ログ・引き継ぎ | `docs/handoff/`, `docs/logs/`(旧 Vault 由来、2026-07-25 移行) | Claude | Chat 切り出し時、状態更新時 |
...
添付すべきファイル:
- CLAUDE.md(この共通ルール)
- docs/claude-collaboration.md(相談ルール)
...
### Claude Design(CD)参照運用ルール

UI 改修 Issue では `docs/claude-design/` を必ず参照すること。

1. **CD ファイルが添付(配置)されていない UI 改修 Issue は着手禁止**
...
5. 詳細正本: `docs/dev-common.md`(旧 `docs/dev_project_common.md`)§ 4
...
Issue 本文には Category C(Issue 起票時に参照すべきもの、`docs/DOCUMENT-MAP.md` § 2 参照)から必要なドキュメントを列挙する。UI 改修 Issue では Category F(CD 修正判定)も「改修分類」ブロックに記載する。

Issue のスコープ判定は `docs/agent-instruction-guide.md` § 1 の抽象度マトリックスに従う。100 行超の Issue は Pre-Issue Recon の要否を Naoya さんに提案する。

新規ドキュメント作成の可能性がある議論では、`docs/DOCUMENT-MAP.md` § 3 の「新規ドキュメント作成判定フロー」を実施し、Category (A-F) 判定と DOCUMENT-MAP.md 更新を Issue 本文に含める。
