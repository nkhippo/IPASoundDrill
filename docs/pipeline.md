...
**パスの正本**: `tools/data-pipeline/paths.py` が canonical paths を定義する。ハードコード文字列より import を優先すること。
...
python3 tools/data-pipeline/generate_flap_ipa.py
python3 tools/data-pipeline/merge_flap_candidates.py
python3 tools/data-pipeline/generate_respelling.py
python3 tools/data-pipeline/merge_respelling.py
python3 tools/data-pipeline/gen_neighbors.py
python3 tools/data-pipeline/merge_neighbors.py
python3 tools/data-pipeline/gen_ga_rp_same.py --report tools/data-pipeline/pipeline/ga_rp_same_report.json
python3 tools/data-pipeline/export_batch_words.py

# After batch merge, if rp_ipa was generated with old happY rules:
python3 tools/data-pipeline/fix_happy_i.py   # word-final /iː/ or /ɪ/ → /i/ (then re-run gen_ga_rp_same)
...
| Script | 役割 |
|--------|------|
| `tools/data-pipeline/phonology_lexicon.py` | 共有語彙リスト（`BATH_WORDS_BASE`, `PALM_WORDS`, `YOD_CORONALS`）— `ga_to_rp.py` と `gen_ga_rp_same.py` から import |
| `tools/data-pipeline/fix_happy_i.py` | rp_ipa の happY 位置 `/iː/`/`/ɪ/` → `/i/` 是正（Phase R2 で1回実行済み。将来バッチ追加時にも実行推奨） |
| `tools/data-pipeline/gen_ga_rp_same.py` | `ga_rp_same` / `ga_rp_same_reason` 一括付与（分類器） |
| `tools/data-pipeline/ga_to_rp.py` | GA→RP ルール変換（**offline fallback のみ**。本番 `rp_ipa` は Claude バッチ同梱） |
| `tools/data-pipeline/gen_rp_ipa.py` | Claude API で RP IPA 生成（新規バッチ用。SYSTEM_PROMPT に happY ルールあり） |

Staging outputs → `tools/data-pipeline/pipeline/`。Neighbors / RP progress → `tools/data-pipeline/derived/`。Merge scripts write `packages/core/data/wordlist.json`。

---

## 2. Phase 2 B2 expansion workflow（バッチ追加の標準手順）

1. Receive `phase2_mN_*_with_gloss.json`（`rp_ipa` 同梱）→ `tools/data-pipeline/batches/`
2. Merge into wordlist（重複スキップ、`_generation_source` 除去、`neighbors: []`）
3. Run pipeline（§1 参照）
4. Verify counts; sync `tools/data-pipeline/derived/rp_progress.json` from wordlist
...
| 優先 | 内容 | 状態 |
|---|---|---|
| 高 | 欠落必須語・屈折形パッチ | 主要語追加済み（`tools/data-pipeline/patches/*_patch.json`） |
...
| 高 | GA バッチ warm（GAS） | `tools/tts/gas/BatchWarm.gs`（5,397語。`docs/tts-design.md` §4） |
