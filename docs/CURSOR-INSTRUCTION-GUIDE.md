...
> **Purpose**: Claude が Cursor に出す指示の抽象度を、コード規模と Issue 性質に応じて適切にコントロールする。Cursor Haiku レベルでも実装可能な粒度で指示を出せるようにする。
...
Claude が Cursor に「現状調査依頼」を発行:

### 4.1 目的

- Claude が index.html 全 3,259 行を取得する代わりに、Cursor に該当箇所だけスキャンさせて MD 化
...
1. Cursor が Recon MD を `docs/cursor/recon/` に配置、コミット + push
...
- **Claude が MCP で先月の PR を取得**、以下を分析:
  ...
  - 抽象度レベル vs Cursor の完了率（中断回数、追加コミット数）
- **見積もり精度が甘い場合**、Claude がガイドライン更新提案（Issue 起票）
- **Naoya 承認後、CURSOR-INSTRUCTION-GUIDE.md 更新**

### 5.3 レビュー観点

- 抽象度「中」で出したのに Cursor が実装できなかった Issue（→ 抽象度「低」に格上げが必要）
- 抽象度「低」で出したのに Cursor が中断しなかった Issue（→ 抽象度「中」で足りる、指示の詳細度が過剰）
- Pre-Issue Recon を実施しなかったが、Cursor が実装中に何度も質問した Issue（→ 該当規模に Recon 必須ルール追加）

## 6. Naoya さんの Chat 内発言と Cursor 指示の橋渡し

Naoya さんが Chat で語る「意思決定の想い」「経緯」「文脈」は、Cursor に直接伝わらない。Claude は以下を実施:

1. Chat 内の Naoya さんの発言を Issue 本文の「背景・目的」5 サブセクションに整理
2. Issue 本文が Cursor 実装レポートに要約される（`docs/DEV-GUARDRAILS.md` § 7 のテンプレート「Issue 背景」セクション）
...
- CURSOR-INSTRUCTION-GUIDE.md 自身も Category A（常時最新化義務）
