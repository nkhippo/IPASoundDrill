...
- **Setup 11 項目（Accent 含む）+ Onboarding**（旧「12 パラメータ」表記は廃止。正本は `docs/design/phase-1/screen-data-mapping.md` §1）
- レイアウト（Phase 1-C）:
...
| `2c` | `.qno` + Progress meter（`#mbSProgress`）+ CEFR / 音 → IPA（音象徴）→ 意味確認（inline）→ 単語+gloss / `#mbSChecks` Mood B。Quiz 凍結（Study-only） |
| `2d` | `#cardDecode` 再利用（`tab=connected`）。Progress / CEFR / `#revealChecks`→`mark:2d:*` は PR1 で自動適用。Type/Level は `3a` |

#### Progress meter（Phase 1-D）

- 配置: `#cardDecode` / `#cardEncode` / `#cardModeBStudy`（`#dProgress` / `#eProgress` / `#mbSProgress`）
...
- 音節境界 `‧`(U+2027): wordlist 0 件のため対象外（将来データ整備後に自動適用）

#### マーキング UI（Phase 1-D）

- `#revealChecks`（`2a`/`2b`/`2d` Reveal）Mood B。`resolveDrillId` で `2d` 振り分け
- `#mbSChecks`（`2c` Study）Mood B（PR2）

#### Reveal 共通要素（Phase 1-D-PR2）

- 反対アクセント: `.alt-ipa*`（`--font-ipa` + `--muted`）
- gloss: `.word-gloss`（`--font-ui` + `--muted`）
- TTS: `.playicon` 全 9 箇所 Mood B
- `.res-ok` / `.res-bad` は現状維持

#### Band 廃止（Phase 1-D-PR2）

- Unlock 系削除: `ept_vocab_band` / `MODEB_BAND_UNLOCK_RATIO` / `getVocabBand` 等
- `MODEB_BANDS` は CEFR 許可リストとして残置（名称リネームは Phase 1-H）
...
- **Exclusive viewport:** `body.vocab-page` / `body.symbol-picker-page`（相互排他、shell 非表示）
- **Sticky filter（`3b`）:** 綴り/IPA segmented、CEFR pill 複数選択（初期全選択）、検索、A–Z
- **Virtualization（Words のみ）:** 固定行高 + scroll window ~20–30 行。Skeleton 維持、time-slicing なし。Phrases は全描画
...
"ept_checks_v1": { "<wordKey>": { "d": 2, "e": 1, "l": 0 } }  // 移行元（残置）
"ept_marks_v1": { "2a:<wordKey>": 2, "2b:<wordKey>": 1 }  // Phase 1-C 正（物理）
"ept_marks_migrated_v1": "1"
// Phase 1 正（論理）: mark:{drill_id}:{word_id} = 0..3
...
- `ept_checks_v1` → `ept_marks_v1` は起動時 lazy（`d→2a` / `e→2b` / `l→2c`、`2d` は 0）。移行後の read/write は marks 側
...
- Decode のみ。UI は `#cardDecode` 再利用（`tab=connected`）。Progress / `#revealChecks`→`mark:2d:*` は Phase 1-D で Mood B
- Reveal 共通: 反対アクセント `.alt-ipa*` / gloss / `.playicon` Mood B（Phase 1-D-PR2）

### 2.8 音から単語を覚える（`2c`）

- Study-only（Quiz 凍結）: 音 → IPA → 意味確認（inline）→ Got It。`#cardModeBStudy` + `#mbSProgress` + `#mbSChecks` Mood B
- Quiz: 凍結（`MODEB_QUIZ_ENABLED=false`）。復活時の distractor は `neighbors`（近傍2＋同 CEFR ランダム1）
- **Band Unlock 廃止（Q-2-B / Phase 1-D-PR2）:** `ept_vocab_band` / `MODEB_BAND_UNLOCK_RATIO` / unlock 関数削除済。`MODEB_BANDS` は CEFR 許可リストとして残置
- プール除外: `letter` / `contraction`

---

## 2b. 語彙ブラウザ（`3b`）+ IPA 記号ピッカー（`3c`）

トップバー `#vocabBtn` から起動する **exclusive full-page**（`<section id="vocabPage">`、`body.vocab-page`）。hash: `#/vocab`（Words）/ `#/vocab/phrases`（Phrases）/ `#/vocab/ipa`（`3c`）。Back（`#vocabBackBtn`）は Menu と独立。

| タブ / 画面 | 内容 |
|------|------|
| **Words** | wordlist 全 **5,397** 語。仮想化リスト・綴り検索・CEFR pills（初期全選択）・A–Z ジャンプ・進捗チェック・CEFR バッジ |
| **Phrases** | `connected_speech.json` 201 句。**非仮想化**。cs_type × level 順。CEFR バッジ |
| **`3c` IPA** | `#symbolPickerPage`（`body.symbol-picker-page`）。Segmented「IPA」または `#/vocab/ipa`。chart パレット + query chips + live IPA substring |

**Sticky filter:** `.filter-bar-sticky` に segmented（綴り/IPA）・CEFR・search を固定。Words 行は 2 段組（単語+バッジ / IPA+gloss）+ チェック + TTS。

詳細: `docs/SPECIFICATION.md` §4.8b–4.8c、`docs/design/phase-1/visual-tokens.md` §5j
...
| 語彙ブラウザ（`3b` full-page / `3c` `#/vocab/ipa` / 仮想化 / sticky filter） | ✅ Phase 1-E PR-1 |
