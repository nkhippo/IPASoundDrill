...
| `docs/agent-reports/README.md` | 実装レポート命名規則・配置ルールの変更、対応エージェント種類の追加 | 変更 Issue の実装エージェントが更新 |
| `docs/agent-reports/TEMPLATE.md` | レポートテンプレート構造の変更（Complexity Retrospective セクションの改変等） | Claude が Issue で提案 → Naoya 承認 |
...
| `AGENTS.md` | 全 AI エージェント共通の実装・PR・レポート運用規約 |
...
| `docs/cursor/reports/` 群 | historical archive（2026-07-20 以前）として必要時に参照 | Naoya + Claude |
| `docs/agent-reports/` 群 | 月次（エージェント混在品質分析用） | Naoya + Claude |
...
2. 内容が「AI エージェント実装レポート」
   → docs/agent-reports/ に配置、個別レポートは Category 対象外
...
|---|---|
| すべて | AGENTS.md（共通運用規約）、CHANGE-CLASSIFICATION.md（分類ブロック構築のため） |
...
|---|---|
| すべて | AGENTS.md, REPOSITORY-STRUCTURE, DEV-GUARDRAILS, .cursor/rules/dev-flow.mdc, CHANGE-CLASSIFICATION.md（分類ブロック確認・Retrospective 実施のため） |
