...
| `docs/agent-reports/README.md` | 実装レポート命名規則・テンプレ参照・対応エージェント種類の変更 | 変更 Issue の実装エージェントが更新 |
| `docs/agent-reports/TEMPLATE.md` | レポートテンプレ構造変更（Retrospective セクション改変等） | Claude が Issue で提案 → Naoya 承認 |
...
| ファイル | Cursor での役割 |
|---|---|
| `AGENTS.md` | 全 AI エージェント共通の実装・PR・実装レポート要件 |
...
| `docs/cursor/reports/` 群 | historical archive（2026-07-20 以前の Cursor 品質分析用） | Naoya + Claude |
...
2. 内容が「AI エージェント実装レポート」
   → docs/agent-reports/ に配置、レポート本体は Category 対象外（README / TEMPLATE は Category A）
