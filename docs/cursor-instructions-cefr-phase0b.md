...
| D4 | フォニックス軸との関係 | **既存 `regField` を維持、新規軸は作らない** |
...
3. JS: `updateSetupFields()` に `cefrField` の表示/非表示制御追加（`reg=regular` 時に非表示）
...
function filteredPool(){
  ...
  if(S.reg==="regular"&&S.grp!=="all")p=p.filter(w=>w.grp===S.grp);
  // CEFR filter: apply only when regField is NOT "regular" (phonics practice
  // has cefr=null on all entries and CEFR selection is meaningless there).
  // When regField is "regular", the CEFR field is hidden in the UI too.
  if(S.reg!=="regular"){
...
**設計注記:**
- CEFR フィルタは `w.cefr && S.cefrLevels.has(w.cefr)` の条件で、`cefr === null` の phonics 語は自動的に除外される
- `S.cefrLevels.size === 0`（ユーザーが全 CEFR ピルを解除した状態）では空プールを返す → `updatePool()` が start ボタンを disabled にする
- 焦点フィルタ（`focus="letters"` など）は既存動作のまま：`letter`/`contraction`/`irregular_verb` 等の src は CEFR = A1（実データ）なので、A1 チェック済みなら通常通り出題される
...
これで `reg=regular` 時に CEFR フィールドが hidden になり、CEFR 対象外のフォニックス練習モードで UI がクリーンになります。
...
function updateSetupFields(){
  ...
  show("cefrField", words && S.reg!=="regular");  // ← 新設
...
| 2 | all | A1 のみ | 1,187 |
| 3 | all | A2 のみ | 1,195 |
| 4 | all | A1, A2, B1 | 2,407 |
| 5 | all | B1 のみ | 25 |
| 6 | all | 全解除 | 0（start disabled） |
| 7 | regular | (CEFR field hidden) | 652（phonics 語全部）または grpField による絞り込み後の数 |
| 8 | irregular | A1, A2 | 2,382 |

シナリオ 7 で CEFR field が非表示になり、既存の grpField 挙動が維持されていることを確認してください。
...
- `updateSetupFields()` に `show("cefrField", words && S.reg!=="regular")` が追加されているか
- CSS `.hidden` クラスが `cefrField` に適用されているか DevTools で確認
