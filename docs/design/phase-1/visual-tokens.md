...
| `--font-mono` | ラベル・言語スイッチャー | `.lang-switcher` / purpose-id |
...
| ヘッダー言語 | `#headerLang` `#langSwitcher` `#langMenu` | 現在言語コード + ▾、メニューは自言語表記 |
| ガイド | `#guideBtn` | 現行 guide 暫定（→ Phase 1-F `3g`） |
| `3h` | `#aboutBlock` | DOM 常時、`about.placeholder` |

---

## §5d Progress meter snapshot（Phase 1-D-PR1）

| 領域 | DOM / CSS | 備考 |
|------|-----------|------|
| バー | `.progress-meter` / `__fill` | height 6px、`width` は inline |
| ラベル | `.progress-meter__label` | `{pct}% ({cur} / {total})` |
| 配置 | `#dProgress` / `#eProgress` | `.qno` 下。Reveal では親カードごと非表示 |
| 割合 | `(S.idx+1) / S.poolTotal` | `updateProgressMeter()` |

## §5e 音象徴視覚化 snapshot（Phase 1-D-PR1）

| 要素 | 実装 | 備考 |
|------|------|------|
| 強勢 `ˈ`/`ˌ` | `.seg.mark` → `var(--signal)` | `2a` / Reveal / `.build` |
| 要注意 | `.seg.nucleus` → `var(--stress)` 下線 | TRAPSET 全適用は対象外 |
| 音節境界 `‧` | 未適用 | wordlist 0 件。将来予約（§4.6） |

## §5f `.build` Mood B トークン化 snapshot（Phase 1-D-PR1）

| セレクタ | 変更 |
|----------|------|
| `.build` | `--hair` / `--panel` / `--radius-button` |
| `.build .ipa` | `--font-ipa` |
| `.build .ph` | `--font-ui` / `--faint` |
| `.kbd` / `.key` / `.kglabel` / `.kbtns` | Mood B トークン参照（レイアウト不変） |

## §5g `2c` Study snapshot（Phase 1-D-PR2）

| 領域 | DOM / 備考 |
|------|------------|
| カード | `#cardModeBStudy` Mood B パネル |
| Progress | `#mbSProgress`（`updateProgressMeter` に `mbS`） |
| CEFR | `.cefr` pill 風 |
| IPA | `--font-ipa` + 音象徴（`buildIpaHtml`） |
| マーキング | `#mbSChecks` Mood B（3 スロット） |
| フロー | Study → 意味表示 inline → Got It（Quiz 凍結） |

## §5h `2d` Connected/Weak snapshot（Phase 1-D-PR2）

| 領域 | 備考 |
|------|------|
| カード | `#cardDecode` + `tab=connected`（専用カードなし） |
| Progress / marks | `#dProgress` / `#revealChecks`→`2d`（PR1 で自動適用） |
| CEFR | 表示のみ |

## §5i Reveal 共通要素 Mood B snapshot（Phase 1-D-PR2）

| 要素 | 実装 |
|------|------|
| 反対アクセント | `.alt-ipa*` → `--font-ipa` / `--muted` |
| gloss | `.word-gloss` → `--font-ui` / `--muted` |
| TTS | `.playicon` 全 9 箇所 → `--signal` / `--hair` / `--panel` |
| 正誤 | `.res-ok` / `.res-bad` 現状維持 |

## §5j `3b` / `3c` 支援画面 snapshot（Phase 1-E PR-1）

| クラス / body | 用途 |
|---------------|------|
| `body.vocab-page` | `#vocabPage` exclusive viewport（shell `.wrap` 非表示） |
| `body.symbol-picker-page` | `#symbolPickerPage` exclusive viewport |
| `.filter-bar-sticky` | `3b`/`3c` 上部 sticky filter（segmented / CEFR / search / chips） |
| `.query-chip` | `3c` query builder の記号チップ |
| `.symbol-cell` | IPA chart パレットの記号セル |
| `.symbol-group-heading` | IPA chart 分類見出し（EN + L1 sub） |
| `.vocab-virt-spacer` | 仮想リスト全高スペーサ |
| `.vocab-virt-window` | 可視ウィンドウ（`translateY`） |
| `.vocab-virt-row` | 固定高の仮想行 |

マッチハイライト: IPA 部分一致箇所に `var(--signal)`。

## §5k `3d` 学習状況 snapshot（Phase 1-E PR-2）

| クラス / body | 用途 |
|---------------|------|
| `body.progress-page` / `.progress-page` | `#learningStatusPage` exclusive viewport |
| `.progress-header` / `.progress-filters` | Back・タイトル・CEFR sticky filter |
| `.progress-overall` | 総合卒業率カード（既存 `.progress-meter` を再利用） |
| `.drill-progress-card` | `2a`–`2d` の縦積み Progress Card |
| `.mark-slot-counts` / `.mark-slot-count` | 0/1/2/3 スロット内訳 |
| `.srs-queue-item` | 単語・IPA・due tag・`2c` direct review 導線 |
| `.srs-virt-spacer` / `.srs-virt-window` | 100件超 queue の fixed-row virtualization |

新規 UI は `--paper` / `--panel` / `--ink` / `--muted` / `--hair` / `--signal` と `--font-ui` / `--font-ipa` / `--font-mono` のみを使用。既存 `.pill-cefr` / `.progress-meter` の意味論は変更しない。
