...
現状の UI 言語は **5 言語**（`en` / `ja` / `zh` / `ko` / `fil`）。
...
| 15 | 実機で当該言語に切替え、**Words / Connected / Weak Forms** 各タブと Mode B、settings、guide を目視 |
...
**2026-06-26 時点の実行結果:** UI 150 キー、音素 43 記号、4 言語、ERROR 0。
...
## 4. キー構造リファレンス（`i18n/en.json` 基準・150 キー）
...
| `lead_html` / `lead_connected_html` / `lead_weak_html` | 各タブのリード文（HTML可） | Weak Forms 追加（2026-06） |
| `tab.*` | Words / Connected Speech / **Weak Forms** | |
