...
| # | 作業 | 手順の正本 | 完了条件 |
|---|------|------------|----------|
| A1 | GAS エディタに最新 `gas/Code.gs` を反映し、**ウェブアプリを新しいバージョンとして再デプロイ** | `gas/README.md`、`cursor/reports/cursor-implementation-report-phase-t.md` §5 | `?urls=1` が Drive 公開 URL を返す |
...
| # | 作業 | 手順の正本 | 完了条件 |
|---|------|------------|----------|
| B1 | リポジトリの `gas/BatchWords.gs`（**5,397 語**）を GAS プロジェクトへ貼り付け | `python3 scripts/export_batch_words.py` → `gas/README.md` | GAS 側リスト件数 = 5,397 |
