...
> **Last updated:** 2026-07-21（Issue #114: governance v2 / agent-reports・.github templates 反映）
...
| **`AGENTS.md`**（repo root） | AI エージェント共通運用ルール | Codex / Cursor / Claude Code の実装フロー、PR、レポート、検証ルール |
| **`.github/PULL_REQUEST_TEMPLATE.md`** | PR description の正本 | PR 本文の必須構成、Complexity Retrospective 確認 |
| **`docs/agent-reports/README.md`** | AI エージェント実装レポート統合先の索引 | 新規実装レポートの配置・命名・過去レポートとの関係 |
| **`docs/agent-instruction-guide.md`** | AI エージェント指示の抽象度ガイド | Pre-Issue Recon、Issue 指示粒度、Cursor-specific notes |
...
**衝突時の優先順位（Product / Runtime 仕様）:** `PURPOSE.md` → `DESIGN.md` → `SPECIFICATION.md` → 本書の運用メモ。

**AI エージェント運用ルール:** `AGENTS.md` と `.cursor/rules/dev-flow.mdc` は Product 仕様系列とは独立して、実装フロー・PR・レポートの手順を規定する。

**履歴ドキュメント:** 新規実装レポートは `docs/agent-reports/` に作成する。`docs/cursor/reports/` 内の古いレポートは historical archive で、当時の語数・パスを引用する場合あり。数値・パスの正本は上表。
...
| **Task history** | `docs/agent-reports/` — Codex / Cursor / Claude Code 等の新規実装レポート。`docs/cursor/` は instructions / briefs / recon と 2026-07-20 以前の historical reports |
...
```
ipasounddrill/
├── AGENTS.md                   # AI エージェント共通運用ルール
├── .github/
│   ├── PULL_REQUEST_TEMPLATE.md
│   ├── ISSUE_TEMPLATE/
│   │   ├── agent-task.md       # AI エージェント向け Issue テンプレート
│   │   ├── bug.md
│   │   ├── docs.md
│   │   └── feature.md
│   └── workflows/
│       ├── trigger-cursor-on-ready.yml
│       ├── label-pr-needs-review.yml
│       ├── approval.yml
│       ├── validate-cefr-tags.yml
│       └── validate-markdown-refs.yml
├── src/
...
│   ├── agent-instruction-guide.md # AI エージェント指示の抽象度ガイド
│   ├── agent-reports/           # AI エージェント実装レポート統合先
│   │   ├── README.md
│   │   └── TEMPLATE.md
│   ├── cursor/                  # Cursor historical archive / instructions / briefs / recon → README.md 参照
│   │   ├── instructions/        # cursor-instructions-*.md
│   │   ├── reports/             # 2026-07-20 以前の historical archive
│   │   ├── briefs/              # 設計相談ブリーフ
...
5. Add `docs/cursor/instructions/`（必要時）+ `docs/agent-reports/` 実装レポート
