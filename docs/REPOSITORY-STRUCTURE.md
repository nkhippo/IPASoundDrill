...
| Layer | Role |
|-------|------|
| **Runtime (Vercel + custom domain)** | `index.html` + JSON/i18n/fonts loaded by the browser |
...
Vercel は main への push で自動デプロイ（`.github/workflows/static.yml` は削除済み、Issue #7 / PR #8 参照）。詳細は `docs/OPERATIONS.md` § 1「Vercel デプロイ」を参照。
