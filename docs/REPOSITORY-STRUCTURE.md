...
| **`docs/agent-reports/README.md`** | AI エージェント実装レポート統合ディレクトリの索引 | 2026-07-20 以降の実装レポート命名規則・テンプレ参照 |
...
**履歴ドキュメント:** `docs/cursor/reports/` は 2026-07-20 以前の historical archive。以降の実装レポートは `docs/agent-reports/` に統合する。古いレポートは当時の語数・パスを引用する場合あり。数値・パスの正本は上表。
...
| **Task history** | `docs/agent-reports/` — AI エージェント実装レポート統合ディレクトリ。`docs/cursor/` は instructions / historical reports / briefs / **recon**（[[pj-2026-07-10-a25d|`docs/cursor/README.md`]]） |
...
│   ├── agent-reports/           # AI エージェント（Codex / Cursor / Claude Code 等）の実装レポート統合ディレクトリ → AGENTS.md 参照
│   ├── cursor/                  # AI タスク履歴 → README.md 参照
│   │   ├── instructions/        # cursor-instructions-*.md
│   │   ├── reports/             # cursor-implementation-report-*.md（2026-07-20 以降は agent-reports/ に統合、historical archive）
│   │   ├── briefs/              # 設計相談ブリーフ
...
| Agent reports | `docs/agent-reports/**`（2026-07-20 以降の実装レポート統合先。要件は `AGENTS.md`） |
| Cursor task docs | `docs/cursor/**`（`reports/` は historical archive。古いレポートは pre-reorg パスを引用する場合あり） |
