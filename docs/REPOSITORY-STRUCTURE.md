...
| **`AGENTS.md`**（repo root） | AI エージェント共通の実装・PR・レポート運用規約 | Issue 対応、変更分類、実装レポート、draft PR の必須手順 |
| **`docs/agent-reports/README.md`** | AI エージェント実装レポートの統合索引 | レポート配置先・命名規則・テンプレート |
...
**履歴ドキュメント:** `docs/cursor/reports/` は 2026-07-20 以前の historical archive。古いレポートは当時の語数・パスを引用する場合がある。2026-07-20 以降の実装レポートは `docs/agent-reports/` に統合し、数値・パスの正本は上表を参照する。
...
│   ├── agent-reports/           # AI エージェント（Codex / Cursor / Claude Code 等）の実装レポート統合ディレクトリ
│   │   ├── README.md            # 配置・命名規則
│   │   └── TEMPLATE.md          # 共通レポートテンプレート
│   ├── cursor/                  # AI タスク履歴 → README.md 参照
│   │   ├── instructions/        # cursor-instructions-*.md
│   │   ├── reports/             # historical archive（2026-07-20 以降は docs/agent-reports/ に統合）
│   │   ├── briefs/              # 設計相談ブリーフ
