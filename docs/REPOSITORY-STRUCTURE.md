...
> **Last updated:** 2026-07-23（Issue #126: `docs/claude-design/` 追加）
...
|------|------|---------------------|
| **`AGENTS.md`** (repo root) | 全 AI エージェント共通の運用規約・検証ルール | PR/レポート/改修分類/Step 2 コメント format の正本 |
...
| **`.github/PULL_REQUEST_TEMPLATE.md`** | PR 本文テンプレート（GitHub 自動挿入） | draft PR 作成時の必須セクション |
| **`docs/agent-reports/README.md`** | AI エージェント実装レポート統合ディレクトリの規約 | 2026-07-20 以降の実装レポート配置・命名 |
| **`docs/cursor/README.md`** | Cursor タスク履歴の索引 | 過去の指示書・Recon の場所（reports は archive） |
...
| **`docs/claude-design/`** | UI/UX の正典（Claude Design 成果物: SP / PC / Design System / favicon） | UI 改修 Issue の視覚仕様参照（セクション ID で画面特定） |
...
**衝突時の優先順位:** `PURPOSE.md` → `DESIGN.md` → `SPECIFICATION.md` → 本書の運用メモ。`AGENTS.md` は **governance 層**（製品仕様系列とは独立。全エージェント運用の正本）。

**履歴ドキュメント:** `docs/cursor/reports/` は 2026-07-20 以前の historical archive。2026-07-20 以降の実装レポート正本は `docs/agent-reports/`（規約は `AGENTS.md`）。古いレポートは当時の語数・パスを引用する場合あり。数値・パスの正本は上表。
...
│   │   ├── reports/             # historical archive（2026-07-20 以前）。以降は docs/agent-reports/
│   │   ├── briefs/              # 設計相談ブリーフ
...
│   │   ├── README.md            # 参照方法・運用ルール
│   │   ├── design-system.dc.html
│   │   ├── sp.dc.html
│   │   ├── pc.dc.html
│   │   ├── favicon.svg
│   │   ├── support.js           # .dc.html 閲覧用ランタイム
│   │   └── update-log.md
...
| `onboarding` | object | Phase 1-F オンボーディング（`slide_1`〜`slide_4`.`title`/`body`、`next`/`skip`/`start`、`title`/`pc_lead`/`guide_aria` 等） |
...
- 新規キー群（Phase 1-F / #149）: `onboarding.slide_1`〜`slide_4`（`title`/`body`）、`onboarding.next` / `skip` / `start`、他ガイド文言
- HTML 埋め込みキーは `_html` サフィックス
- 動的置換プレースホルダ: `{n}`, `{band}`, `{pct}`, `{m}`, `{t}`, `{c}`, `{list}`, `{p}`, `{sy}`, `{s}`, `{a}`
- LS 追加キー（#149）: `onboarding_completed_v1`（`"true"` で初回オンボ完了。スキップも完了扱い）

_Last synced with code: 2026-07-24（Issue #150 PC header follow-up）_
