...
| `computeSrsQueue` | `ept_hist_v1` / `ept_vocab_v1` を word dedupe + earliest dueAt で統合 |
| `renderProgressSrsQueue` / `paintProgressSrsWindow` | 100件超 queue の progress 専用 virtualization |
| `openDrillProfile` | `3d` のドリルカードから既存 `3a` 導線へ移動 |
| `startDirectStudy` | SRS 行の該当語1件を `2c` Study として開始 |
