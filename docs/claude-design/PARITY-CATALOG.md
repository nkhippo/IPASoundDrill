...
- **app-5**: 3b 語彙リスト CD準拠フルリワーク(フレーズタブ削除【Q6】/ 3×3 ドット削除【Q7】/ 意味表示追加 / 最大3の IPA 部分一致フィルタ / IPA キーボード横スクロール / 戻る丸アイコン)。SP/PC 両方
  ...
  - ✅ **PC(3b-pc)= 単一カラムで確定**(2026-07-27 Naoya): カードは SP と同一リワークがレスポンシブ反映済(単一カラム 820px、CD カード意匠一致、SRS 無し=Q7、●●● 非表示=app-4)。**app 側変更不要**。残タスクは CD `#3b-pc` 側の是正(**CD-7 SRS削除 / CD-8 ●●●削除** → `cd-updates/2026-07-27_cd-parity-fixes-pc3b.md`、Claude Design round-trip)。2カラム化は将来別途。
- **app-6** ✅済: 3e「IPA って何？」フルページ overlay(`#ipaInfoPage`, CD #3e 準拠)+ 1a フッター「IPAって何?」ピル(signal強調)【Q8】。commit `867e42f`
- **app-7** ✅済: 3f 言語設定フルページ overlay(`#languagePage`, CD #3f 準拠, 6言語ラジオ+アクセント注記)+ フッター「言語設定」ピル【Q9】。言語切替=setLang(サブディレクトリ reload/hash 保持)。commit `867e42f`
...
|---|---|---|---|
| 3e IPA って何？ | フッター「IPAって何?」→ #3e | **✅ app-6 済**: `#ipaInfoPage` overlay + フッター導線(signal強調)。CD #3e 視覚一致 | 完了 |
| 3f 言語設定 | フッター「言語設定」→ #3f フルページ | **✅ app-7 済**: `#languagePage` overlay(6言語)+ フッター導線。CD #3f 視覚一致。言語切替往復確認 | 完了(Q9=フルページ新設で決着) |
...
CD `#3b-pc`(pc.dc.html 236-)を実装しようとすると、**確定済の決定・完了済 app と 3 点衝突**していた。SP は矛盾なく完了。PC カード意匠は SP と同一で反映済:
