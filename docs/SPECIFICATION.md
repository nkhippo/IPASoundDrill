...
**最終更新:** 2026-07-09（無制限セッション・先読みキュー・振り返りボタン）  
...
| **振り返り** | 各カード内の小ボタン（1 問目以降・Reveal 含む）。押下でサマリー表示（再開なし） |
| **終了** | プール全問消化で自動サマリー。Menu でセットアップへ戻ると途中統計は破棄 |
...
**出題:** フィルタ後プールの全語を Study として重複なしで消化（§2.3b の先読み・振り返りと同じ）。
...
- UI 言語: **en / ja / zh / ko / fil**（`localStorage.app_lang`、**167 キー**）
...
| メーター | `3 / 50`（プール総数固定）または `done`（サマリー時） |
| 振り返り | 各カード右上（`data-reflect`）。Reveal 画面にも表示 |
...
正答率・`{c} / {t} 正解`（`t` = 採点済み回答数）・苦手音素・復習リスト・**TOPへ**（`#againBtn` → セットアップ）/ Review misses only。プール全問消化でも同一画面へ自動遷移。振り返りボタンからの手動遷移時も再開なし。
