...
UI i18n とは独立。各言語キー（`en`, `ja`, `ko`, `zh-Hans`, `zh-Hant`, `fil`）に8セクション（`welcome` … `how_to_use`）。`welcome` は4段落のナラティブ。`philosophy` は3段落（知覚-産出原則・辞書発音・SLM/PAM 根拠）。`solves` は2段落（3ギャップの具体例・設計としてのスコープ）。モーダルで閲覧。
