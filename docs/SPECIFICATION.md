...
**最終更新:** 2026-07-10（Phase R・進捗チェック・Phrases CEFR バッジ・語彙 5,397）  
...
topbar の `#vocabBtn` から起動。プレイ中も利用可。Words（wordlist **5,397語**）/ Phrases（connected_speech 201句）タブ。弱形 36 語は Phrases タブに含まない。Words は A→Z ソート・検索（debounce 120ms）・A–Z ジャンプ。各行: 単語 / GA+RP IPA / 意味（`vocabDisplayGloss`）/ 品詞 / TTS / **進捗チェック（d/e/l）**。RP 行は常に表示。`ga_rp_same` 時は `/ipa/（同じ）` 形式。英語 UI では `gloss.en === w` の自己参照を `def` または `(品詞)` で代替。Phrases は cs_type × level 順、タイプバッジ + **CEFR バッジ**。モバイル（`max-width: 599px`）では検索欄を非表示。Escape で閉じる。i18n: `vocab.*`（5キー × 6言語）。
