...
**最終更新:** 2026-07-16（Q-7-A: Connected `cs_rule` 6 言語化）  
...
- 連結句・弱形ルール文（`cs_rule`）: en / ja / fil / **ko / zh-Hans / zh-Hant**（連結句 201 件は 6 言語。弱形 36 件は当面 en/ja/fil）
...
|----------|----------|--------|---------------|
| `#exitConfirmModal` | `#exitConfirmScrim` | **No 相当**（練習継続） | scrim → No 相当 |
| `#settingsModal` | `#settingsScrim` | 閉じる | scrim → close |
| `#guideModal` | `#guideScrim` | 閉じる | scrim / Close → close |

##### Modal Keyboard Interaction

- **Escape キー**: 3 モーダル（`#exitConfirmModal`, `#settingsModal`, `#guideModal`）すべてで対応（統合 `keydown` リスナー、優先順位 Exit > Settings > Guide）
  - `#exitConfirmModal`: No 相当（練習継続、`closeExitConfirm()`）
  - `#settingsModal`: 閉じる（`closeSettings()`）
  - `#guideModal`: 閉じる（`closeGuide()`）

この判断は 2026-07-16 の Q-9-A で確定（詳細: Vault `30_projects/IPASoundDrill/design-decisions.md` § Q-9）。
