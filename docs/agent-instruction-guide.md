...
> **Last updated**: 2026-07-21（Issue #114: agent-agnostic 化）
> **Purpose**: Naoya / Claude が Codex・Cursor・Claude Code などの AI エージェントに出す指示の抽象度を、コード規模と Issue 性質に応じて適切にコントロールする。エージェント選択に依らず実装可能な粒度で Issue を設計できるようにする。
...
- Claude が index.html 全 3,259 行を取得する代わりに、実装エージェントに該当箇所だけスキャンさせて MD 化
...
1. 実装エージェントが Recon MD を `docs/cursor/recon/` に配置、コミット + push（現行 path は historical compatibility として保持）
...
- **Naoya 承認後、agent-instruction-guide.md 更新**

### 5.3 レビュー観点

- 抽象度「中」で出したのに実装エージェントが完了できなかった Issue（→ 抽象度「低」に格上げが必要）
- 抽象度「低」で出したのに実装エージェントが中断しなかった Issue（→ 抽象度「中」で足りる、指示の詳細度が過剰）
- Pre-Issue Recon を実施しなかったが、実装エージェントが実装中に何度も質問した Issue（→ 該当規模に Recon 必須ルール追加）

## 6. Naoya さんの Chat 内発言とエージェント指示の橋渡し

Naoya さんが Chat で語る「意思決定の想い」「経緯」「文脈」は、実装エージェントに直接伝わらない。Claude は以下を実施:

1. Chat 内の Naoya さんの発言を Issue 本文の「背景・目的」5 サブセクションに整理
2. Issue 本文が AI エージェント実装レポートに要約される（`docs/agent-reports/TEMPLATE.md` の「Issue 背景」セクション）
...
## 8. Cursor-specific notes

- `.cursor/rules/dev-flow.mdc` は Cursor 固有の開発フロールールとして維持する
- 既存の `docs/cursor/recon/` は Pre-Issue Recon の historical path として保持する
- 2026-07-20 以前の `docs/cursor/reports/` は historical archive として扱い、新規実装レポートは `docs/agent-reports/` に作成する

## 9. 運用の継続性

- `docs/agent-instruction-guide.md` 自身も Category A（常時最新化義務）
