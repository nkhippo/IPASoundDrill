...
**Drive パブリック共有:** 音声ファイル（`*_v2.mp3`, `*_v4.mp3`, `*_weak_v2.mp3`）は `saveToDrive_` 時に `ANYONE_WITH_LINK` に設定される。クライアントは `?urls=1` で URL を取得後、`https://drive.google.com/uc?export=download&id=...` から直接 fetch する。既存ファイルの一括公開は GAS エディタで `migratePublicSharing` を実行（約 5.5 分で PAUSED → 再実行を繰り返し、ログが DONE になるまで）。途中再開用に Script Properties（`MIGRATE_PUBLIC_*`）を使う。最初からやり直す場合は `resetMigratePublicSharing()`。
