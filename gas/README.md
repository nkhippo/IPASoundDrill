...
**Drive パブリック共有:** 音声ファイル（`*_v2.mp3`, `*_v4.mp3`, `*_weak_v2.mp3`）は `saveToDrive_` 時に `ANYONE_WITH_LINK` に設定される。クライアントは `?urls=1` で URL を取得後、`https://drive.google.com/uc?export=download&id=...` から直接 fetch する。

既存ファイルの一括公開は `migratePublicSharing`（GAS **6 分上限**のため複数チャンクが必要。Drive `setSharing` は約 50–80 件/分）:

| 方法 | 手順 |
|------|------|
| **推奨・自動** | `installMigratePublicTrigger(5)` を 1 回実行 → 放置。DONE でトリガー自動削除 |
| 手動 | `migratePublicSharing` を PAUSED のたびに再実行 |
